...
module.exports = function (client, options) {
	class LavalinkMusic { // Construct everything?
		constructor(clientt, options) {
			...
			clientt.player = null;
			this.helpCmd = (options && options.helpCmd) || 'mhelp';
			...
			this.stopCmd = (options && options.stopCmd) || 'stop';
			...
			this.seekCmd = (options && options.seekCmd) || 'seek';
...
"${music.prefix}${music.seekCmd}" - Seeks through the queue!
	...
	music.play = async (message, suffix) => {
		const msg = message;
